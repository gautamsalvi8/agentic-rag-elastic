...
# Supabase: sessions via REST API (works with any key format; Streamlit Cloud pe login persist)
def _get_supabase_config():
    url = (os.environ.get("SUPABASE_URL") or "").strip().rstrip("/")
    key = (os.environ.get("SUPABASE_KEY") or os.environ.get("SUPABASE_SERVICE_KEY") or "").strip()
    if not key and hasattr(st, "secrets"):
        key = (getattr(st.secrets, "SUPABASE_KEY", None) or getattr(st.secrets, "SUPABASE_SERVICE_KEY", None) or "").strip()
    if not url and hasattr(st, "secrets"):
        url = (getattr(st.secrets, "SUPABASE_URL", None) or "").strip().rstrip("/")
    if url and key:
        return url, key
    return None

def _supabase_session_insert(url: str, key: str, token: str, username: str, user_db_key: str, auth_provider: str) -> bool:
    try:
        import requests
        r = requests.post(
            f"{url}/rest/v1/app_sessions",
            headers={"apikey": key, "Authorization": f"Bearer {key}", "Content-Type": "application/json", "Prefer": "return=minimal"},
            json={"token": token, "username": username or "User", "user_db_key": user_db_key or "", "auth_provider": auth_provider or "google"},
            timeout=10,
        )
...
def _supabase_session_select(url: str, key: str, token: str):
    try:
        import requests
        r = requests.get(
            f"{url}/rest/v1/app_sessions",
        ...
        if r.status_code == 200 and r.json() and len(r.json()) > 0:
            return r.json()[0]
        return None
    except Exception:
        return None

def _supabase_session_delete(url: str, key: str, token: str) -> bool:
    try:
        import requests
        r = requests.delete(
            f"{url}/rest/v1/app_sessions",
...
def _create_url_session_token(username: str, db_key: str, provider: str) -> str:
    ...
    if cfg:
        url, key = cfg
        if _supabase_session_insert(url, key, token, username or "User", db_key or "", provider or "google"):
            return token
...
def _validate_url_session_token(token: str):
    ...
    if cfg:
        url, key = cfg
        row = _supabase_session_select(url, key, t)
            ...
            if created:
                try:
                    ...
                    if (datetime.now(_tz.utc) - created_dt) > timedelta(days=URL_SESSION_EXPIRY_DAYS):
                        _supabase_session_delete(url, key, t)
                        return None
                except Exception:
                    pass
            return {
...
def _delete_url_session_token(token: str):
    ...
    if cfg:
        url, key = cfg
        if _supabase_session_delete(url, key, t):
            return
    sessions = _load_url_sessions()
    sessions.pop(t, None)
    _save_url_sessions(sessions)
...
def _load_users():
    if os.path.exists(USER_DB):
        try:
            ...
            # Corrupted or partial write — avoid crash; treat as empty
            return {}
    return {}
...
def _save_user_history():
    ...
    if db_key not in users:
        users[db_key] = {}
    raw = st.session_state.get("conversations", {}) or {}
    # Persist doc_files safely by base64-encoding bytes so history (including PDFs) survives refresh/restart.
    sanitized: dict[str, dict] = {}
    for cid, conv in raw.items():
        c = dict(conv)
        doc_files = c.get("doc_files") or {}
        if doc_files:
            encoded_files = {}
            for name, data in doc_files.items():
                if isinstance(data, (bytes, bytearray)):
                    try:
                        encoded_files[name] = base64.b64encode(data).decode("utf-8")
                    except Exception:
                        # Fallback: drop this entry if encoding fails
                        continue
                else:
                    encoded_files[name] = data
            c["doc_files"] = encoded_files
        sanitized[cid] = c
    users[db_key]["history"] = sanitized
    _save_users(users)
...
# Load per-user conversation history once per session.
# IMPORTANT: Sirf history list load karo, active chat hamesha "clean" new chat se start ho.
if st.session_state.get("authenticated") and not st.session_state.get("_history_loaded"):
    saved_history = _load_user_history()
    if saved_history:
        # History screen ke liye conversations store karo, lekin
        # messages/docs/doc_files/focus_doc ko yahan se auto-restore mat karo.
        st.session_state.conversations = saved_history
    # Fresh session → hamesha ek naya, empty chat (no cross-chat bleed).
    ...
    if st.session_state.conversations:
        for conv_id, conv in reversed(list(st.session_state.conversations.items())[-10:]):
            if st.button(conv['title'], key=f"conv_{conv_id}", use_container_width=True):
                ...
                # Decode base64-encoded doc_files for this conversation
                _df = {}
                for _name, _data in (conv.get("doc_files") or {}).items():
                    if isinstance(_data, str):
                        try:
                            _df[_name] = base64.b64decode(_data.encode("utf-8"))
                        except Exception:
                            _df[_name] = _data
                    else:
                        _df[_name] = _data
                st.session_state.doc_files = _df
                st.session_state._pending_docs_for_badge = None
                st.session_state.last_response = None
...
# CHAT VIEW (DEFAULT)
    ...
    if not st.session_state.messages:
        ...
        with input_col:
            ...
            st.caption("_ElasticAI can make mistakes, so proper filename-based retrieval is recommended while writing._")
        with icons_col:
                                ...
                                try:
                                    with st.spinner("Indexing..."):
                                        ...
                                        if not (text or "").strip():
                                            st.warning("No text could be extracted from this PDF. It may be empty or image-only.")
                                        elif not st.session_state.get("ingestor"):
        ...
        elif send_clicked and not (draft or "").strip():
    ...
    if user_input:
        if not st.session_state.docs:
            ...
            try:
                ...
                # If user says "this doc/this document" or mentions a specific filename,
                # bias retrieval to that document only (avoid cross-contamination across multiple PDFs).
                target_docs_from_phrase: list[str] = []
                all_docs = list(st.session_state.get("docs", []) or [])
                if all_docs:
                    last_doc = all_docs[-1]
                    if any(phrase in q_lower for phrase in ["this doc", "this document", "about this doc", "about this document"]):
                        target_docs_from_phrase = [last_doc]
                    else:
                        for _doc_name in all_docs:
                            name_lower = str(_doc_name).lower()
                            base, _ = os.path.splitext(name_lower)
                            if (base and base in q_lower) or name_lower in q_lower:
                                target_docs_from_phrase.append(_doc_name)
                ...
                generic_followups = [
                    "anything more", "anything else", "aur batao", "aur btao",
                    "tell me more", "more?", "aur?",
                    # Treat "sources" follow-ups as generic so we reuse last context instead of running a fresh search.
                    "sources", "sources?", "show sources", "source?", "source"
                ]
                ...
                # Normalize query for search: strip fillers (okk, please, etc.) so "okk , qualification ?" → "qualification"
                _qnorm = re.sub(r"[^\w\s]", " ", query_lower)
                _qnorm = re.sub(r"\s+", " ", _qnorm).strip()
                for _filler in ("okk ", "ok ", "okay ", "please ", "pls ", "tell me ", "can you ", "could you ", "what is the ", "what is ", "whats the ", "just "):
                    if _qnorm.startswith(_filler):
                        _qnorm = _qnorm[len(_filler):].strip()
                for _filler in (" okk", " ok", " okay", " please", " pls"):
                    if _qnorm.endswith(_filler):
                        _qnorm = _qnorm[:-len(_filler)].strip()
                search_query = (_qnorm if _qnorm and len(_qnorm.strip()) > 1 else retrieval_query).strip() or retrieval_query
                # Incomplete questions like "what is the" (nothing after) → use broad fetch so LLM can ask to complete
                _ql = query_lower.strip()
                is_incomplete = _ql in ("what is the", "what is", "whats the", "tell me the", "give me the") or (
                    _ql.startswith("what is the ") and len(_ql.split()) <= 4
                )
                ...
                _broad_keywords = [
                    ...
                    # Natural-language summary prompts like "tell me about this doc"
                ...
                # Self-profile markers: queries about "my" qualifications, experience, resume, profile.
                _self_profile_markers = [
                    "my qualifications", "my qualification",
                    "my work experience", "my experience",
                    "my resume", "resume", "cv",
                    "do you have my resume", "about my resume", "about resume",
                    "about me", "my profile", "profile",
                    "mera experience", "meri qualification", "meri qualifications",
                ]
                is_self_profile_query = any(m in query_lower for m in _self_profile_markers)
                focus_doc = st.session_state.get("focus_doc")
                # Cross-doc: "resume and embedded", "based on my resume and X knowledge" → search ALL docs
                _cross_doc_markers = ("embedded", "arduino", "microcontroller", "knowledge", " and ")
                is_cross_doc_query = (
                    ("resume" in query_lower or "cv" in query_lower)
                    and any(c in query_lower for c in _cross_doc_markers)
                ) or "based on my resume and" in query_lower
                # Very short queries (1-3 words) are likely broad/exploratory
                is_short = len(query_lower.split()) <= 3
                # For summaries / incomplete questions: use broad fetch so we don't show "No relevant documents"
                # For specific questions: fetch 8 with reranker; use normalized query so "okk , qualification ?" → "qualification"
                ...
                with st.spinner("🔍 Searching documents..."):
                    if not st.session_state.get("searcher"):
                        ...
                        # Cross-doc: "resume + embedded" etc → search ALL docs so LLM can synthesize
                        if is_cross_doc_query and st.session_state.get("docs"):
                            k_cross = num_docs
                            print(f"\n🔎 [FRONT] Cross-doc query detected. search_all for all docs → k={k_cross}")
                            resp = st.session_state.searcher.search_all(
                                filenames=st.session_state.docs,
                                k=k_cross,
                            )
                        # Self-profile queries (about "my" resume/experience) → focus_doc ko directly search_all se fetch karo.
                        elif is_self_profile_query and focus_doc:
                            k_self = num_docs
                            print(f"\n🔎 [FRONT] Self-profile query detected. search_all for focus_doc={focus_doc} → k={k_self}")
                            resp = st.session_state.searcher.search_all(
                                filenames=[focus_doc],
                                k=k_self,
                            )
                        elif is_summary or is_greeting or (is_incomplete and st.session_state.get("docs")):
                            # Greeting, summary, or incomplete ("what is the"): broad fetch so LLM can answer or ask to complete
                            k_greet = 5 if is_greeting else (num_docs if is_summary else 10)
                            print(f"\n🔎 [FRONT] {'Greeting' if is_greeting else 'Summary' if is_summary else 'Incomplete query'} mode: search_all for docs={st.session_state.docs} → k={k_greet}")
                            resp = st.session_state.searcher.search_all(
                                filenames=st.session_state.docs,
                                k=k_greet,
                            )
                            # If summary returned no results, retry once (e.g. ES hiccup) so "summarise this doc" works consistently
                            if is_summary and st.session_state.docs and (resp.get("results") or []) == []:
                                print("\n🔎 [FRONT] Summary search_all returned 0, retrying once...")
                                resp = st.session_state.searcher.search_all(
                                    filenames=st.session_state.docs,
                                    k=k_greet,
                                )
                        else:
                            print(f"\n🔎 [FRONT] Searching for: '{search_query}' (normalized) → k={num_docs}, reranker={use_reranker}")
                    ...
                    # Otherwise, if the query explicitly refers to "this doc" or a filename, prefer that set.
                    current_target_docs = []
                    if st.session_state.get("messages"):
                        last_msg = st.session_state.messages[-1]
                        current_target_docs = last_msg.get("docs") or []
                    if not current_target_docs and 'target_docs_from_phrase' in locals() and target_docs_from_phrase:
                        current_target_docs = list(dict.fromkeys(target_docs_from_phrase))  # de-duplicate, preserve order
                    ...
                    metrics = {
                        ...
                        # Store both pre- and post-filter counts for better debug + guardrails
                    ...
                    # Strict: only chunks from docs uploaded in THIS chat — no cross-contamination (case-insensitive so "Resume.pdf" matches "resume.pdf")
                    session_docs = set(st.session_state.get("docs", []) or [])
                    session_docs_lower = {str(d).lower(): d for d in session_docs}
                    if session_docs:
                        session_filtered = [
                            r for r in results
                            if (r.get("source") or r.get("metadata", {}).get("filename", "") or "").lower() in session_docs_lower
                        ]
                        results = session_filtered

                    # After filtering: if no results, decide based on TRUE retrieval count (before filtering)
                    if not results:
                        retrieval_count = len(raw_results)

                        # 1️⃣ No documents retrieved at all → genuine "no relevant documents"
                        if retrieval_count == 0:
                            ...
                            elif not st.session_state.get("searcher"):
                                st.session_state.messages.append({
                                    "role": "assistant",
                                    "content": "⚠️ Search backend is unavailable. Please check that Elasticsearch is running and backend dependencies (e.g. hybrid_search) are loaded."
                                })
                                st.session_state.last_response = {**metrics, "results": []}
                            elif is_summary and session_docs:
                                # Fallback: extract text from uploaded PDFs (doc_files) and generate summary without ES
                                doc_files = st.session_state.get("doc_files", {}) or {}
                                extracted_texts = []
                                fallback_chunks = []
                                for idx, doc_name in enumerate(session_docs):
                                    data = doc_files.get(doc_name)
                                    if data:
                                        try:
                                            ...
                                            if text.strip():
                                                ...
                                                # Also create a pseudo-chunk so Sources panel still has something to show
                                                fallback_chunks.append({
                                                    "text": text[:500],
                                                    "score": 1.0,
                                                    "chunk_id": f"pdf-fallback-{idx}",
                                                    "metadata": {"filename": doc_name},
                                                    "source": doc_name,
                                                })
                                        except Exception:
                                            pass
                                if extracted_texts and st.session_state.get("generator"):
                                    combined = "\n\n---\n\n".join(extracted_texts)[:100000]
                                    history = "\n".join([
                                        f"User: {m['content']}" if m['role'] == 'user' else f"Assistant: {m['content']}"
                                        for m in st.session_state.messages[-4:]
                                    ])
                                    with st.spinner("💬 Generating summary from uploaded documents..."):
                                        answer = st.session_state.generator.generate(
                                            query=user_input, docs=[combined], history=history, top_score=1.0
                                        )
                                    st.session_state.messages.append({"role": "assistant", "content": answer})
                                    # Use fallback_chunks so Sources + scores are still visible even without ES results
                                    st.session_state.last_response = {**metrics, "results": fallback_chunks, "primary_source": None}
                                    st.session_state.conversations[st.session_state.conversation_id] = {
                                        "messages": st.session_state.messages.copy(),
                                        "title": st.session_state.messages[0]['content'][:40] + "..." if st.session_state.messages else "New Chat",
                                        "docs": list(st.session_state.get("docs", [])),
                                    ...
                                    _reason = "Document content is not available in this session (e.g. after refresh or switching chats)." if not (st.session_state.get("doc_files") or {}) else "Elasticsearch may be unavailable or the index may not have the document yet."
                                    st.session_state.messages.append({
                                        "role": "assistant",
                                        "content": f"I couldn't generate a summary right now. {_reason} Please re-upload the document in this chat and ask for a summary again."
                                    })
                                    st.session_state.last_response = {**metrics, "results": []}
                            else:
                                st.session_state.messages.append({
                        ...
                        # 2️⃣ Retrieval found docs, but filters removed them (e.g., doc-name mismatch).
                        #    In this case, fall back to using the top raw_results, BUT still restricted to this chat's docs.
                        else:
                            print(f"⚠️ [FRONT] Filters removed all {retrieval_count} retrieved docs. Falling back to raw_results within session_docs (ignoring target_docs).")
                            # Prefer non-empty text, then cap to num_docs
                            non_empty_raw = [r for r in raw_results if (r.get('text', '') or '').strip()]
                            candidate_results = (non_empty_raw or raw_results)[:num_docs]

                            # Always restrict strictly to docs uploaded in THIS chat
                            if session_docs:
                                candidate_results = [
                    ...
                    # If STILL no results but docs are present in this chat, do a raw PDF fallback:
                    # read all PDFs from doc_files and let the LLM answer from combined text.
                    if not results:
                        doc_files = st.session_state.get("doc_files", {}) or {}
                        session_docs = set(st.session_state.get("docs", []) or [])
                        if doc_files and session_docs and st.session_state.get("generator"):
                            extracted_texts = []
                            fallback_chunks = []
                            for idx, doc_name in enumerate(session_docs):
                                data = doc_files.get(doc_name)
                                if data:
                                    try:
                                        ...
                                        if text.strip():
                                            extracted_texts.append(text)
                                            # Pseudo-chunk so Sources panel still shows something sensible
                                            fallback_chunks.append({
                                                "text": text[:500],
                                                "score": 1.0,
                                                "chunk_id": f"pdf-fallback-{idx}",
                    ...
                    # Use the ES hybrid score for top_score instead of treating it as a hard reject signal.
                    score = float(
                        results_for_answer[0].get('score', results_for_answer[0].get('rerank_score', 0.0))
                    ) if results_for_answer else 0.0

                    # Soft safety only: if score is extremely low, log it and mark in metrics,
                    # but do NOT block generation.
                    LOW_CONFIDENCE_THRESHOLD = 0.15
                    low_conf = score < LOW_CONFIDENCE_THRESHOLD
                    metrics["low_confidence_retrieval"] = bool(low_conf)
                    if low_conf:
                        print(f"⚠️ low_confidence_retrieval=True | score={score:.4f} | query={retrieval_query}")
